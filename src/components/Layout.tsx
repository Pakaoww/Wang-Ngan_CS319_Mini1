...
const links: { page: Page; label: string }[] = [
  ...
  { page: "certification", label: "Certification" },
...
export default function Layout({
  ...
  return (
    <div className="site">
      ...
      <footer>
        <div>
          <strong>Wang Ngan</strong>
          <p>AI and Digital Career & Learning Platform</p>
        </div>
        <div>
          <strong>Explore</strong>
          <button onClick={() => onNavigate("jobs")}>Jobs</button>
          <button onClick={() => onNavigate("training")}>Training</button>
        </div>
        <div>
          <strong>Support</strong>
          <button onClick={() => onNavigate("events")}>Events</button>
          <button onClick={() => onNavigate("news")}>News & Articles</button>
